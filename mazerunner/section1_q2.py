...
# generate path for DFS
def pathGenDFS(c):
    global final_path_dfs
    final_path_dfs=[]
    pl=0
    while c!=0:
        final_path_dfs.append(c)
        pl+=1
        c=parent(c)
    return pl

# generate path for BFS
def pathGenBFS(c):
    global final_path_bfs
    final_path_bfs=[]
    pl=0
    while c!=0:
        final_path_bfs.append(c)
        pl+=1
        c=parent(c)
    return pl

# generate path for ASTAR
def pathGenASTAR(c):
    global final_path_astar
    final_path_astar=[]
    pl=0
    while c!=0:
        final_path_astar.append(c)
        pl+=1
        c=parent(c)
    return pl

# function to generate fringes for A*
# Accepts maze, queue and dimension as input
# returns fringes
...
# function to run A*
# accepts Maze and dimension as input
...
def update(i):
    ...
    if (i == len(final_path_bfs)+len(final_path_dfs)+len(final_path_astar) - 1):
        break_counter = 1
    if (break_counter == 0):
        temp_maze = initial_maze
        # set the path to be displayed as a frame of animation
        if (i < len(final_path_bfs)):
            temp_maze[final_path_bfs[i]] = 0.25
        elif ((i >= len(final_path_bfs)) & (i < (len(final_path_dfs) + len(final_path_bfs)))):
            temp_maze[final_path_dfs[i - len(final_path_bfs)]] = 0.30
        else:
            temp_maze[final_path_astar[i - len(final_path_bfs) - len(final_path_dfs)]] = 0.35
...
# generate a maze and create multiple copies to run searches on.
M1=MazeGen(dim,0.2)
M2 = copy.deepcopy([x if x == 0 else 0.4 for x in M1])
M3 = copy.deepcopy([x if x == 0 else 0.4 for x in M1])
# maze copy for visualization
initial_maze = copy.deepcopy([x if x == 0 else 0.4 for x in M1])
...
# run dfs search
ltn=DFS_Agent(M3,dim)
pathGenDFS(ltn)

# add starting and last elements to results
final_path_bfs.append(0)
final_path_bfs.reverse()
final_path_bfs.append((dim**2)-1)

final_path_dfs.append(0)
final_path_dfs.reverse()
final_path_bfs.append((dim**2)-1)

final_path_astar.append(0)
final_path_astar.reverse()
final_path_bfs.append((dim**2)-1)
...
if (path == 0):
    ...
    ani = animation.FuncAnimation(fig, update, frames=(len(final_path_bfs)+len(final_path_dfs)+len(final_path_astar)), interval=100)
